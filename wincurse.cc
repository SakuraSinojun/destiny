...
fin.open("data\\FONTDATA");
 if (!fin.is_open()){
    ...
    WindowWidth= 800;   //(55 + (OPTIONS[OPT_VIEWPORT_X] * 2 + 1)) * fontwidth;
    WindowHeight= 600; //(OPTIONS[OPT_VIEWPORT_Y] * 2 + 1) *fontheight;
    WindowX= 0; // (GetSystemMetrics(SM_CXSCREEN) / 2)-WindowWidth/2;    //center this
    WindowY= 0; //(GetSystemMetrics(SM_CYSCREEN) / 2)-WindowHeight/2;   //sucker
